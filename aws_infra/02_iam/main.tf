...
resource "aws_iam_role_policy_attachment" "aws10_ssm_attach" {
  role       = aws_iam_role.aws10_ec2_role.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore"
}

# [필수] S3 정책 연결: 배포 파일 업로드/다운로드용
resource "aws_iam_role_policy_attachment" "aws10_s3_attach" {
  role       = aws_iam_role.aws10_ec2_role.name
...
resource "aws_iam_role_policy_attachment" "aws10_codedeploy_attach" {
  role       = aws_iam_role.aws10_codedeploy_service_role.name
